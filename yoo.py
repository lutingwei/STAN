...
test64 = pickle.load(open('datasets/yoochoose1_64/test.txt', 'rb'))
train64 = pickle.load(open('datasets/yoochoose1_64/train.txt', 'rb'))
...
for item in test64[1]:
    item_set.add(item)
item_list = sorted(list(item_set))  # 17745个item
...
train_x = torch.Tensor(train_seqs)  # torch.Size([369859, 145])
train_pos = torch.Tensor(train_poses)
train_y = torch.Tensor(target_seqs)
test_x = torch.Tensor(test_seqs)  # torch.Size([55898, 145])
...
for w in w_list:
    ...
    model = DualAttention(100, 100, 17746, 145, w, dropout=0.5, activate='relu').cuda()  # w=20
        ...
        for step, (x_train, pos_train, y_train) in enumerate(train_dataload):
            ...
            if (step + 1) % 100 == 0:
                print("[%02d/%d] [%03d/%d] mean_loss : %0.2f" % (
                    epoch + 1, 50, step, len(train_sets) / 512, losses / step + 1))
