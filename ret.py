...
test64 = pickle.load(open('datasets/retailrocket/test.txt', 'rb'))
train64 = pickle.load(open('datasets/retailrocket/train.txt', 'rb'))
...
for item in test64_y:
    item_set.add(item)  # 36968个item

# %%

train_x = torch.Tensor(train_seqs)  # [433648, 284]
train_pos = torch.Tensor(train_poses)  # [433648, 284]
train_y = torch.Tensor(target_seqs)  # [433648]
test_x = torch.Tensor(test_seqs)  # [15132, 284]
test_pos = torch.Tensor(test_poses)  # [15132, 284]
test_y = torch.Tensor(target_test_seqs)  # [15132]
...
for w in w_list:
    ...
    model = DualAttention(100, 100, 36969, 284, w, atten_way='MLP', decoder_way='trilinear2', dropout=0.5).cuda()
    ...
    for epoch in range(50):
        ...
        with torch.no_grad():
            ...
            print("best result: " + str(best_result_))
            print("==================================")
