...
with open(ds, "r") as f:
    ...
    for line in lines:
        ...
        if sess_id in sess_clicks:
            sess_clicks[sess_id] += [item]   #一个sess_id对应多个item
            sess_date[sess_id] = timestamp   #一个sess_id对应多个timestamp
        else:
    ...
    if dataset == 'diginetica':
        for i in list(sess_clicks):
            #sorted_clicks按照日期、时间排序  得到的结果还是item,timestamp, timeframe
...
pickle.dump(train_data, open('datasets/diginetica/train.txt', 'wb'))
pickle.dump(test_data, open('datasets/diginetica/test.txt', 'wb'))

#pickle.dump(train_data, open('datasets/retailrocket/train.txt', 'wb'))
#pickle.dump(test_data, open('datasets/retailrocket/test.txt', 'wb'))
